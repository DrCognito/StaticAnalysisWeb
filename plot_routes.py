...
def draft(team, dataset, side):
    ...
    draft = pc.Drafts(app.current_dir / app.metadata_dict[team]['path'],
                      dataset)
...
def wards(team, dataset, side):
    app.update_meta_dict()
    wards = pc.Wards(app.current_dir / app.metadata_dict[team]['path'],
                     dataset)
    navigators = app.get_team_nav(team, dataset)
    try:
...
def wards_separate(team, dataset, side):
    app.update_meta_dict()
    wards_separate = pc.WardsSeparate(app.current_dir / app.metadata_dict[team]['path'],
                                      dataset)
    navigators = app.get_team_nav(team, dataset)
    try:
...
def positioning(team, dataset, side):
    app.update_meta_dict()
    positioning = pc.Positioning(app.current_dir / app.metadata_dict[team]['path'],
                                 dataset)
    navigators = app.get_team_nav(team, dataset)
    try:
...
def smoke(team, dataset, side):
    app.update_meta_dict()
    smoke = pc.Smoke(app.current_dir / app.metadata_dict[team]['path'],
                     dataset)
    navigators = app.get_team_nav(team, dataset)
    try:
...
def scan(team, dataset, side):
    app.update_meta_dict()
    scan = pc.Scan(app.current_dir / app.metadata_dict[team]['path'],
                   dataset)
    navigators = app.get_team_nav(team, dataset)
    try:
